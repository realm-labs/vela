...
#[test]
fn lsp_references_find_schema_record_variant_field_labels_and_patterns() {
...
pub fn main(state: QuestState) -> i64 {
    ...
    let response = response_value(server.handle_json(&request(
        ...
        serde_json::json!({
            ...
            "position": {
                "line": 0,
                "character": line(schema_text, 0).find("count").expect("schema field")
            },
    ...
    assert_reference(
        ...
        &schema_uri,
        0,
        line(schema_text, 0).find("count").expect("schema field"),
    );
    assert_reference(
        references,
        &uri,
    ...
    assert_reference(
        ...
        &uri,
        6,
        line(text, 6).find("count").expect("pattern field label"),
    );
