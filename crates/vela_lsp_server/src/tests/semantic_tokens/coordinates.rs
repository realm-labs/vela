...
fn start(root: &Path) -> (TestServer, Value) {
...
fn full(server: &mut TestServer, uri: &str) -> Value {
...
fn rows(document: &Document, data: &Value, legend: &Value) -> Vec<oracle::Row> {
...
fn apply_delta(previous: &Value, delta: &Value) -> Value {
...
fn delta(server: &mut TestServer, uri: &str, previous_id: &Value) -> Value {
