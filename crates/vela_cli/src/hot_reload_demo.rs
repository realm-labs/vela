...
use vela_hot_reload::{HotReloadRuntime, compile_initial, compile_update};
...
pub(crate) fn run(initial_path: &str, updated_path: &str) -> Result<(), Box<dyn Error>> {
    ...
    let initial =
        compile_initial(SourceId::new(1), &initial_source).map_err(|error| format!("{error:?}"))?;
    let mut runtime = HotReloadRuntime::new(initial);
    let old = runtime.current();
    let old_before = run_main(&old.to_program())?;

    let update = compile_update(&old, SourceId::new(2), &updated_source)
    ...
    println!(
        "old_version={} new_version={} old_before={old_before:?} \
