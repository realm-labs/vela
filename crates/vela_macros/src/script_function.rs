...
fn expand_result(attr: TokenStream, input: TokenStream, mode: FunctionMode) -> Result<TokenStream> {
    ...
    let meta = function_meta(&item, attrs, docs, mode)?;
...
mod tests {
    ...
    fn rejects_missing_function_name() {
        ...
        .expect_err("missing function name should fail macro expansion");
    ...
    fn infers_value_keyed_map_and_set_signature_hints() {
        ...
        .expect("value-keyed map function should expand")
        .to_string();
