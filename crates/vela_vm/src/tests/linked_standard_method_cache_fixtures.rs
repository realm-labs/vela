...
pub(super) use array::{
    linked_array_clear_cache_program, linked_array_contains_cache_program,
    linked_array_distinct_cache_program, linked_array_first_cache_program,
    linked_array_get_cache_program, linked_array_index_of_cache_program,
    linked_array_insert_cache_program, linked_array_is_empty_cache_program,
    linked_array_join_cache_program, linked_array_last_cache_program,
    linked_array_max_cache_program, linked_array_min_cache_program, linked_array_pop_cache_program,
...
pub(super) fn linked_set_has_record_identity_cache_program() -> LinkedMethodCacheFixture {
    let method_id = vela_stdlib::std_method_id("Set", "has").expect("Set::has method id");
    let mut program = vela_bytecode::LinkedProgram::new();
    let main_name = program.intern_debug_name("main");
    let method_name = program.intern_debug_name("has");
    let record_name = program.intern_debug_name("Player");
    let field_name = program.intern_debug_name("level");
    let record_ty = program.push_type(vela_bytecode::LinkedType::new(
        vela_def::TypeId::new(0x715),
        record_name,
    ));
    let set_from_array_name = program.intern_debug_name("set::from_array");
    let set_from_array = program.push_native_function(vela_bytecode::LinkedNativeFunction::new(
    ...
    let level = code.push_constant(Constant::Scalar(vela_common::ScalarValue::I64(8)));
    code.push_instruction(vela_bytecode::linked::Instruction::new(
        vela_bytecode::linked::InstructionKind::LoadConst {
            dst: Register(0),
            constant: level,
