...
fn cyclic_owned_export_returns_a_typed_error() {
    ...
    let HeapValue::Array(values) = heap.get_mut(root).unwrap() else {
        panic!()
    };
    values.push(Value::HeapRef(child));
    let error = persistent_value_to_owned(&Value::HeapRef(root), &mut heap).unwrap_err();
...
fn shared_values_are_exported_as_independent_tree_branches() {
    ...
    assert_eq!(
        persistent_value_to_owned(&Value::HeapRef(root), &mut heap).unwrap(),
...
fn deep_owned_export_stops_before_recursive_conversion_or_drop_overflows() {
    ...
    drop(persistent_value_to_owned(&Value::HeapRef(root), &mut heap).unwrap());
    root = heap.allocate(HeapValue::Array(vec![Value::HeapRef(root)]));
    assert_eq!(
