...
impl SetKey {
    ...
    pub(super) fn matches_value(
        ...
        match (self, value) {
            ...
            (Self::Int(lhs), Value::I64(rhs)) => Ok(*lhs == *rhs),
            (Self::Float(lhs), Value::F64(rhs)) if rhs.is_finite() => Ok(*lhs == rhs.to_bits()),
            ...
            (_, Value::Null | Value::Bool(_) | Value::I64(_)) => Ok(false),
            (_, Value::F64(value)) if value.is_finite() => Ok(false),
            _ => type_error(operation),
        }
    }
