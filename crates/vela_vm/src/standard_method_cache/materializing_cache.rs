...
pub(super) fn call_cached_map_materialization(
    receiver: &Value,
    target: StandardMethodInlineCacheTarget,
    args: &[Value],
    heap: &mut Option<&mut HeapExecution<'_>>,
    ...
    match target {
        StandardMethodInlineCacheTarget::Keys => {
            ...
            Some(make_string_array(payload, heap, budget, "method keys"))
        }
        StandardMethodInlineCacheTarget::Values => {
            ...
            Some(make_array(payload, heap, budget, "method values"))
        }
...
fn make_map_entry(
    ...
    let key = make_string(key, heap, budget, operation)?;
