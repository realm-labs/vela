...
fn main() {
    ...
    let bool_value_type = reflect::type_of(true).unwrap_or(missing_type);
    let i8_type = reflect::type_info("i8");
    let i16_type = reflect::type_info("i16");
    let i32_type = reflect::type_info("i32");
    let int_value_type = reflect::type_of(42).unwrap_or(missing_type);
    ...
    let float_value_type = reflect::type_of(1.5).unwrap_or(missing_type);
    let string_value_type = reflect::type_of("quest").unwrap_or(missing_type);
    let char_value_type = reflect::type_of('q').unwrap_or(missing_type);
    let array_value_type = reflect::type_of(["quest"]).unwrap_or(missing_type);
    let map_value_type = reflect::type_of({"quest": 1}).unwrap_or(missing_type);
    let set_value_type = reflect::type_of(set::from_array(["quest"])).unwrap_or(missing_type);
    let option_value_type = reflect::type_of(option::some(1)).unwrap_or(missing_type);
    let result_value_type = reflect::type_of(result::ok(1)).unwrap_or(missing_type);
    let range_value_type = reflect::type_of(1..3).unwrap_or(missing_type);
    ...
    return reflect::has_function("math::max")
        ...
        && type_of_checks
        && reflect::docs(math).unwrap_or("") == "Deterministic math standard-library helpers."
        && reflect::docs(option_module).unwrap_or("") == "Option standard-library propagation helpers."
        && reflect::docs(result_module).unwrap_or("") == "Result standard-library propagation helpers."
        && reflect::docs(set_module).unwrap_or("") == "Set standard-library construction helpers."
        && reflect::docs(bytes_module).unwrap_or("") == "Bytes standard-library conversion helpers."
        && reflect::attr(math, "stdlib").unwrap_or("") == "math"
        && reflect::attr(option_module, "stdlib").unwrap_or("") == "option"
        && reflect::attr(result_module, "stdlib").unwrap_or("") == "result"
        && reflect::attr(set_module, "stdlib").unwrap_or("") == "set"
        && reflect::attr(bytes_module, "stdlib").unwrap_or("") == "bytes"
        && reflect::attr(string_type, "stdlib").unwrap_or("") == "builtin"
        && reflect::attr(bytes_type, "stdlib").unwrap_or("") == "builtin"
        && reflect::attr(char_type, "stdlib").unwrap_or("") == "builtin"
        && reflect::attr(option_type, "stdlib").unwrap_or("") == "option"
        && reflect::attr(result_type, "stdlib").unwrap_or("") == "result"
        ...
        && trim.owner == "string"
        && reflect::returns(trim).unwrap_or("") == "String"
        && reflect::attr(trim, "stdlib").unwrap_or("") == "string"
        ...
        && reflect::returns(bytes_to_hex).unwrap_or("") == "String"
        && reflect::attr(bytes_to_hex, "stdlib").unwrap_or("") == "bytes"
        && reflect::returns(bytes_values).unwrap_or("") == "Iterator"
        && reflect::returns(char_to_string).unwrap_or("") == "String"
        && reflect::attr(char_to_string, "stdlib").unwrap_or("") == "char"
        && reflect::returns(char_is_ascii_digit).unwrap_or("") == "bool"
        ...
        && array_push.params[0].type.unwrap_or("") == "Any"
        ...
        && reflect::returns(range_len).unwrap_or("") == "i64"
        && reflect::attr(range_len, "stdlib").unwrap_or("") == "range"
        && range_is_empty.params.is_empty()
        && reflect::returns(range_is_empty).unwrap_or("") == "bool"
        ...
        && reflect::returns(iterator_collect_array).unwrap_or("") == "Array"
        && iterator_collect_set.params.is_empty()
        && reflect::returns(iterator_collect_set).unwrap_or("") == "Set"
        ...
        && reflect::attr(option_variants[0], "stdlib").unwrap_or("") == "option"
        && option_variants[0].fields[0].name == "0"
        && option_variants[0].fields[0].type.unwrap_or("") == "Any"
        && reflect::docs(option_variants[0].fields[0]).unwrap_or("") == "Dynamic Option::Some payload value."
        && reflect::attr(option_variants[0].fields[0], "stdlib").unwrap_or("") == "option"
        && option_variants[1].name == "None"
        && reflect::docs(option_variants[1]).unwrap_or("") == "Represents expected absence without a payload."
        && reflect::attr(option_variants[1], "stdlib").unwrap_or("") == "option"
        && result_variants.len() == 2
        && result_variants[0].name == "Ok"
        && reflect::docs(result_variants[0]).unwrap_or("") == "Carries a successful Result payload."
        && reflect::attr(result_variants[0], "stdlib").unwrap_or("") == "result"
        && result_variants[0].fields[0].type.unwrap_or("") == "Any"
        && reflect::docs(result_variants[0].fields[0]).unwrap_or("") == "Dynamic Result::Ok payload value."
        && reflect::attr(result_variants[0].fields[0], "stdlib").unwrap_or("") == "result"
        && result_variants[1].name == "Err"
        && reflect::docs(result_variants[1]).unwrap_or("") == "Carries a recoverable Result error payload."
        && reflect::attr(result_variants[1], "stdlib").unwrap_or("") == "result"
        && result_variants[1].fields[0].type.unwrap_or("") == "Any"
        && reflect::docs(result_variants[1].fields[0]).unwrap_or("") == "Dynamic Result::Err payload value."
        && reflect::attr(result_variants[1].fields[0], "stdlib").unwrap_or("") == "result"
        ...
        && reflect::attr(max, "stdlib").unwrap_or("") == "math"
        && reflect::attr(some, "stdlib").unwrap_or("") == "option"
        && reflect::attr(ok, "stdlib").unwrap_or("") == "result"
        && reflect::attr(set_from_array, "stdlib").unwrap_or("") == "set"
        && reflect::attr(bytes_from_hex, "stdlib").unwrap_or("") == "bytes"
        && reflect::docs(max).unwrap_or("") == "Returns the larger numeric value."
        && reflect::docs(sqrt).unwrap_or("") == "Returns the square root as a float."
        && reflect::docs(some).unwrap_or("") == "Wraps a value in Option::Some."
        && reflect::docs(ok).unwrap_or("") == "Wraps a success value in Result::Ok."
        && reflect::docs(set_from_array).unwrap_or("") == "Builds a set from array values."
        && reflect::docs(bytes_from_hex).unwrap_or("") == "Decodes hexadecimal text to bytes or returns an error string."
        && reflect::returns(max).unwrap_or("") == "Any"
        && reflect::returns(sqrt).unwrap_or("") == "f64"
        ...
        && set_params[0].type.unwrap_or("") == "Array"
        && bytes_params.len() == 1
        && bytes_params[0].name == "text"
        && bytes_params[0].type.unwrap_or("") == "String";
}
"#,
        )
        .expect("program should compile");
