...
use vela_common::{
    FieldId, HostMethodId, HostObjectId, HostTypeId, SourceId, TraitId, TypeId, VariantId,
};
use vela_host::{HostPath, HostRef, HostValue, MockStateAdapter, PatchOp, PatchTx};
use vela_reflect::{FieldDesc, MethodDesc, TraitDesc, TypeDesc, TypeKey, VariantDesc};
use vela_vm::{ExecutionBudgetKind, VmError, VmResult};
use vela_vm::{HostExecution, Value, VmErrorKind};

use crate::{
    CallOptions, EffectSet, Engine, FunctionAccess, NativeFunctionDesc, NativeFunctionId,
    NativeMethodDesc, Runtime, ScriptArgsExt, TypeHint,
};
