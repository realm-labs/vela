...
#[test]
fn override_metadata_is_explicit_but_does_not_change_callable_abi() {
    let plain = compile_test_program(
        SourceId::new(508),
        "pub fn patched(value: i64) -> i64 { return value + 1; }",
    )
    .expect("plain callable");
    let overridden = compile_test_program(
        SourceId::new(509),
        r#"
#[override(host::game::increment)]
fn patched(value: i64) -> i64 { return value + 1; }
"#,
    )
    .expect("override callable");
    let plain = plain.binding_schema().callables().next().expect("plain");
    let override_callable = overridden
        .binding_schema()
        .callables()
        .next()
    ...
    assert_eq!(
        plain.contract_fingerprint,
        override_callable.contract_fingerprint
    );
    assert_ne!(
        plain.public_path, "host::game::increment",
        "the override target remains separate from Vela callable identity"
    );
}

#[test]
fn malformed_override_attributes_fail_at_the_declaration() {
    for (source, expected) in [
        (
            "#[override] pub fn patched(value: i64) -> i64 { return value; }",
            "requires exactly one host callable path",
        ),
        (
            "#[override(game::increment)] pub fn patched(value: i64) -> i64 { return value; }",
            "must begin with `host::`",
        ),
        (
            "#[override(target = host::game::increment)] pub fn patched(value: i64) -> i64 { return value; }",
            "positional host callable path",
        ),
        (
            "#[override(host::game::increment)] #[override(host::game::other)] pub fn patched(value: i64) -> i64 { return value; }",
            "duplicate `override` attributes",
        ),
    ] {
        let error = compile_test_program(SourceId::new(510), source)
            .expect_err("malformed override must fail compilation");
