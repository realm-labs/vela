...
#[test]
fn syntax_only_path_comparison_preserves_record_trait_diagnostic() {
    let source = SourceId::new(1);
    let text = r#"
struct Score { amount: i64 }

fn main(left: Score, right: Score) {
    return left < right;
}
"#;
    let semantic = parse_semantic_source(source, text).expect("source should parse");
...
"#;
    ...
    compiler
        .compile_body_payload_statements_for_test(&payload.body)
        .expect("syntax-only field return body should compile");
    ...
    assert!(
        ...
        "CST field return should emit a field read"
    );
}

#[test]
fn syntax_only_nested_field_let_body_compiles_without_owned_body_lookup() {
    let source = SourceId::new(1);
    let text = r#"
fn main(object) {
...
"#;
    ...
    compiler
        .compile_body_payload_statements_for_test(&payload.body)
        .expect("syntax-only nested field let body should compile");
    ...
    assert!(
        compiler
            ...
            .count()
            >= 2,
        "CST nested field let should emit each field read"
    );
}
...
#[test]
fn syntax_only_array_let_body_compiles_without_owned_body_lookup() {
    let source = SourceId::new(1);
    let text = r#"
fn main(input) {
    let values = [input, input.amount, input + 1];
}
"#;
    let semantic = parse_semantic_source(source, text).expect("source should parse");
    let (mut compiler, payload) = cst_payload_compiler_for_function(&semantic, "main");
    ...
    compiler
        .compile_body_payload_statements_for_test(&payload.body)
        .expect("syntax-only array let body should compile");
    ...
    assert!(
        ...
        "CST array let should emit array construction"
    );
}

#[test]
fn syntax_only_map_return_body_compiles_without_owned_body_lookup() {
    let source = SourceId::new(1);
    let text = r#"
fn main(input) {
...
"#;
    ...
    compiler
        .compile_body_payload_statements_for_test(&payload.body)
        .expect("syntax-only map return body should compile");
    ...
    assert!(
        compiler
            ...
            .instructions
            .iter()
            .any(|instruction| matches!(instruction.kind, UnlinkedInstructionKind::MakeMap { .. })),
        "CST map return should emit map construction"
    );
}
