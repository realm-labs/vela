...
pub fn main() { source_any().grant(1, 2) }"#;
    ...
    #[test]
    fn signature_help_resolves_source_method_on_source_method_return() {
        let document = DocumentId::from("/workspace/scripts/game/main.vela");
        let text = r#"
            struct Player { level: i64 }
        ...
        let main_line = text.lines().nth(9).expect("main line should exist");
        let argument_offset = main_line
            .find("2)")
            .expect("second argument should exist in chained method call");
        let position = Position::new(9, argument_offset);
        let help = databases
            .signature_help(&document, position)
            .expect("signature help should resolve source method on method-return receiver");
    ...
    #[test]
    fn signature_help_resolves_schema_method_on_schema_method_return() {
        let document = DocumentId::from("/workspace/scripts/game/main.vela");
        let text = r#"
            pub fn main(player: Player) { player.inventory().grant(1, 2) }
        "#;
        ...
        let help = databases
            .signature_help(&document, position)
            .expect("signature help should resolve schema method on method-return receiver");
    ...
    #[test]
    fn signature_help_resolves_schema_trait_method_on_schema_function_return() {
        let document = DocumentId::from("/workspace/scripts/game/main.vela");
        let text = r#"
            pub fn main() { current_reward().preview(1, 2) }
        "#;
        let files = vec![SourceFileSnapshot::new(document.clone(), text)];
        let config = WorkspaceConfig::workspace([WorkspaceRoot::from("/workspace/scripts")]);
        let project = assemble_project_sources(&config, &files, &Workspace::new().snapshot());
        ...
        let help = databases
            .signature_help(&document, position)
            .expect("signature help should resolve schema trait method on returned receiver");
    ...
    #[test]
    fn signature_help_resolves_schema_trait_method_on_schema_method_return() {
        let document = DocumentId::from("/workspace/scripts/game/main.vela");
        let text = r#"
            pub fn main(player: Player) { player.rewardable().preview(1, 2) }
        "#;
        let files = vec![SourceFileSnapshot::new(document.clone(), text)];
        let config = WorkspaceConfig::workspace([WorkspaceRoot::from("/workspace/scripts")]);
        let project = assemble_project_sources(&config, &files, &Workspace::new().snapshot());
        ...
        let help = databases
            .signature_help(&document, position)
            .expect("signature help should resolve schema trait method on method-return receiver");
