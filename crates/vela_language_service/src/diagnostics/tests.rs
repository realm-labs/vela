...
#[test]
fn analysis_diagnostics_report_tuple_destructuring_arity_mismatch() {
    let document = DocumentId::from("/workspace/scripts/game/main.vela");
    let mut db = LanguageServiceDatabases::new();
    db.update(&project(&[file(
        document.as_str(),
        "\
pub fn main() {
    let (first, last, extra) = (\"Ada\", 1)
    let (name, score) = (\"Grace\", 2)
}",
    )]));
        ...
        diagnostics.diagnostics().iter().any(|diagnostic| {
            diagnostic.code() == Some("analysis::tuple_arity_mismatch")
                ...
                && diagnostic.labels().iter().any(|label| {
                    label
                        .message()
                        .contains("tuple destructuring arity does not match initializer")
                })
        }),
        "{:?}",
        diagnostics.diagnostics()
