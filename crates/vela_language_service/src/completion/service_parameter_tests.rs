...
#[test]
fn service_parameter_matrix_preserves_contextual_types_and_owned_edits() {
        ...
        .flat_map(|crlf| ["full", "missing", "ordinary"].map(|mode| (crlf, mode)))
    {
        ...
        for case in spec.oracle["queries"].as_array().expect("queries") {
            ...
            if case.get("signature").is_some() {
                let help = db.signature_help(
                    &uri(file),
                    position(&source.text, source.markers["cursor"].start.byte),
                );
                assert_eq!(
                    serde_json::json!(help.as_ref().map(|h| h.signatures()[0].label())),
                    if mode == "full" {
            ...
            for expected in expected {
                ...
                if let Some(symbol) = expected["symbol"].as_str() {
                    assert_eq!(
                        item.symbol(),
                        Some(&CompletionSymbol::Schema(symbol.to_owned()))
                    );
...
#[test]
fn service_parameter_facts_refresh_after_schema_changes() {
    let spec = load("completion-service-parameters");
    let mut fixture = FixtureWorkspace::new(&spec).expect("fixture");
    let original = fixture.disk.remove("schema.json").expect("schema").text;
    ...
    for (schema, detail) in [
        ...
        for target in [&mut db, &mut fresh] {
            if let Some(schema) = schema {
                target.load_schema_artifact_json("/workspace/schema.json", schema);
            } else {
                target.clear_schema();
            }
        }
        let actual = db.completion_items(&uri("scripts/local.vela"), cursor);
        assert_eq!(actual.items().len(), 1);
        assert_eq!(actual.items()[0].detail(), detail);
        assert_eq!(
            actual,
            fresh.completion_items(&uri("scripts/local.vela"), cursor)
        );
        let builds = db.analysis_facts_build_count();
        assert_eq!(
            actual,
            db.completion_items(&uri("scripts/local.vela"), cursor)
        );
        assert_eq!(db.analysis_facts_build_count(), builds);
    }
}
