...
|---|---|---|
| Source calls, defaults and named arguments | `completion-named-arguments`, `call-parameter-mapping`, `call-argument-context`, `completion-call-expressions`: occupied/current/future slots, nested delimiters, labels versus values, active/expected parameters, edits. | Combine callable ownership with actual dependency packages and same-path declarations. |
| Source methods and traits | `completion-members`, `completion-callable-hints`, `completion-callable-returns`: inherent/trait/default/Provider methods, hints and returned members. | Cross-package receiver identity must be checked beyond package-less display names; source-backed docs and navigation must agree. |
...
| Imports and unavailable owners | `callable-imports`, `completion-import-aliases`, `completion-expression-ownership`: source/schema/stdlib aliases, local/declaration shadowing, private/missing/duplicate owners. | Direct dependency function/method identities, not only the existing dependency type-position matrix. |
...
1. Check dependency callable/member identity and cross-feature ownership next.
