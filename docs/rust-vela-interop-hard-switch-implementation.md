...
| `vela_vm` | Execute explicit release; check the full active scoped-resource table at every await. |
| `vela_host` | Retain group invalidation and parent/child checks; add reviewed typed erased reborrow. |
| `vela_engine` | Retain root teardown and origins; provide root-local typed Service invocation authority. |
| `vela_macros` | Generate complete typed `base` thunks; delete placeholder arms; reject incomplete signatures. |
| `vela_reflect` | Preserve scoped return facts; do not expose `base` or typed reborrow internals. |
| language service/LSP | Show scoped capability types, await errors, release hints, and explicit code actions. |
...
`ReleaseBorrowLease` remains a valid explicit MIR/bytecode/VM operation. The
switch deletes automatic producers, not the operation used by authored source.
...
- retain explicit `host::release` lowering and VM execution;
...
only authored host::release produces ReleaseBorrowLease in new artifacts
...
- update hover, signature help, completion, inlay hints, and code actions; and
...
| AW-04 | explicit release permits await |
...
1. Can an author identify every scoped value requiring explicit release?
