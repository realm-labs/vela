...
closure calls with default baseline data, includes a cache-enabled callback
collection row with warmed inline caches and bytecode profile counters, and
splits host-boundary rows into field read/write, nested path read/write, RMW
mutation, dynamic key access, and host method calls. M20 reports must separate
