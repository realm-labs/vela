...
cache_delta rows with checksum_match and delta_kind when the harness emits
paired cache-enabled rows; delta_kind separates true cache-hit rows from
profile-only hot-offset rows, and native-call cache-enabled rows count resolved
