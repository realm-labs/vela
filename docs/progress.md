...
uses explicit reserved packages. Engine now loads a sealed package/HIR snapshot,
...
functions, const initializers, trait defaults, impl methods, lambdas, and
...
Parameter-default call lowering now also reads callee source spans from HIR call
facts and no longer checks callee syntax paths in its support predicate.
HIR bodies now also record field/member receiver expression facts keyed by
...
facts, and the old parsed member-access site helper has been removed. HIR
...
with local literal/range fast paths explicitly opting out of path folding
instead of reconstructing syntax path segments.
...
reconstructing a syntax-only block body during lambda lowering. General nested
...
the bytecode syntax payload/dispatcher are deleted, and D1 stable identity,
...
malformed tokens, unknown text, and source spans are preserved in the syntax
record. HIR, analysis, bytecode, language-service, LSP, and formatter paths use
rowan syntax records, typed wrappers, HIR facts, or compiler-owned facts rather
than the deleted owned-AST body parser.

The final legacy audits are clean for the Rowan track: no active code under
`crates/`, `examples/`, or editor packages contains `legacy-body-parser`,
`legacy_body_parser`, `parse_owned_body_blocks_for_tests`, old owned body AST
imports, `.fallback()` payload accessors, bytecode `cst_payload` /
`syntax_only` / `is_syntax_only` scaffolding, or token-gap formatter production
symbols. Remaining `cst` terminology is local to `vela_syntax` concrete syntax
tree implementation/tests and architecture documents; downstream compiler,
...
than unit sentinels. Runtime-owned
...
- M19.5 has started with native call operands: compiled native calls can carry
  ...
  Ordinary and host bytecode index reads, host index path construction, and
  indexed record-shape helpers now also use HIR index/call facts as their
