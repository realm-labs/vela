...
M20.5 Phase 14 update: selected struct fields, enum record fields, and adjacent
nested declaration member groups now use parser-owned member spans for
context-aware range formatting through both the language service and native LSP
server, preserving enclosing indentation while limiting edits to the selected
member group. Remaining formatting follow-up is narrower: broader multi-line
on-type reflow and dynamic-boundary hint suppression for future hint families
remain open.
