...
nested declaration member groups, and completed multi-line top-level items now
use parser-owned spans for context-aware formatting through both the language
service and native LSP server. Remaining formatting follow-up is narrower:
broader nested on-type reflow and dynamic-boundary hint suppression for future
hint families remain open.
