...
- M19.5 has started with native call operands: compiled native calls can carry
  ...
  name fallback. Host field/path reads, writes, and method calls are routed
  through a focused VM host-access boundary, giving later path-key or
  direct-adapter work one replacement point.
