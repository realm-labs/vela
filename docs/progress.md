...
- An M19 managed-heap host read conversion benchmark coverage checkpoint is
  recorded in [performance.md](performance.md):
  `managed_heap_host_read_conversion` now measures host map, record, and enum
  aggregates being read into managed heap values and used through map indexing,
  array joins, record fields, and enum pattern matching.
...
- An M19 string constant register reuse checkpoint is recorded in
  [performance.md](performance.md): repeated string constant loads reuse a
  matching heap string already held in the destination register before falling
  back to the existing allocation path.
- An M19 short-array construction checkpoint is recorded in
  [performance.md](performance.md): heap-mode array literal construction now
  uses length-specialized register reads for zero through eight elements before
  falling back to the general preallocated path.
