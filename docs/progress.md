...
- Remaining M19.5/M20 cache-entry gaps after the primitive refactor:
  ...
  - callback and closure allocation costs now have isolated quick and default
    baseline rows; M20 cache-enabled stdlib method, callback collection, and
  ...
  - runtime bytecode offset counters cover linked hot offsets and hot-reload
    invalidation; cache-enabled stdlib, callback collection, and host-boundary
    rows now consume those counters, and follow-on M20 work needs broader
    cache-enabled benchmark rows;
