...
- Continue the M20.5 native LSP capability track by closing the remaining
  Phase 3 scale gaps: avoid full HIR rebuilds for single-file body edits,
  expand synthetic timing/memory evidence toward larger workspaces, then begin
  the diagnostics query slice without advertising incomplete LSP capabilities.
