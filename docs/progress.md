...
outside `vela_syntax` isolated to the bytecode legacy payload boundary.
...
Source-less statement missing-child payload helpers now build syntax-only
statement payloads without accepting expression fallback `Expr`s, removing the
old statement-to-expression fallback constructor used only by CST fixtures.
...
Expression-backed host path receivers and host method fallback receivers now
prefer rowan CST receiver payloads for host root type and field/method owner
lookup before falling back to temporary legacy receiver expressions.
...
before falling back to temporary legacy expression spelling.
