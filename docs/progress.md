...
nested declaration member groups, completed multi-line top-level items, and
completed nested impl/trait methods now use parser-owned spans for
context-aware formatting through both the language service and native LSP
server. Remaining formatting follow-up is narrower: broader non-method nested
