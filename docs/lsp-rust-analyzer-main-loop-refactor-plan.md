...
`crate::...` paths or a better module boundary over `super::super` imports.
Avoid re-exports unless they define a deliberate scoped API or ownership
facade; do not add re-exports merely to shorten imports or hide module
placement. Do not flatten the crate into one directory of unscoped files;
organize modules by ownership scope such as transport, main loop, state,
handlers, protocol conversion, reload, profiling, and tests. Keep ordinary
active source and test files under 1200 lines unless there is a concrete
documented exception or splitting would make the logic materially less
coherent; when a file approaches or exceeds that threshold, split it by
...
- [x] Retry retryable stale requests once using a fresh snapshot.
  - Retryable `textDocument/completion` requests now run on the latency task
    ...
    `completionItem/resolve`, `textDocument/semanticTokens/full`,
    `textDocument/documentSymbol`, `textDocument/foldingRange`, and
    `workspace/symbol` now use the same retryable task metadata and shared
...
- [x] Delete or empty production use of custom protocol params in
  ...
  - Legacy document symbol, folding range, and workspace symbol query decoding
    ...
    `cargo test -p vela_lsp_server folding`.
  - Legacy selection range, semantic tokens full/delta/range, and inlay hint
    query decoding now use `lsp_types` params and `lsp/from_proto.rs`; their
    custom protocol param structs have been removed. Validated with
    `cargo test -p vela_lsp_server selection_range`,
...
- [ ] Delete production use of custom `RequestId`, `JsonRpcMessage`,
  ...
  - Typed dispatcher error projection paths now use a `JsonRpcResult`
    ...
    `cargo fmt --all -- --check`, and
    `cargo clippy -p vela_lsp_server --all-targets -- -D warnings`.
  - Legacy lifecycle and configuration notification-request rejection paths
    now use `JsonRpcResult` constructors instead of importing standalone
    `success_response` or `error_response`; initialize and shutdown success
    responses also route through the same constructor. Validated with
    `cargo test -p vela_lsp_server lifecycle`,
    `cargo test -p vela_lsp_server workspace_folders`,
    `cargo test -p vela_lsp_server schema_reload`,
    `cargo fmt --all -- --check`, and
  ...
  - Typed `GlobalState` and `GlobalStateSnapshot` request handlers now route
    ...
    `cargo fmt --all -- --check`, and
    `cargo clippy -p vela_lsp_server --all-targets -- -D warnings`.
  - Legacy query handlers now route success and invalid-parameter responses
    through `JsonRpcResult` constructors instead of importing standalone
  ...
  - Temporary `JsonRpcResult` payloads now carry typed `lsp_server::Response`
    ...
    typed main loop sends messages directly. At that checkpoint, the only
    remaining raw response payload was the JSON-RPC parse-error `id: null`
    escape hatch. Validated with
    `cargo test -p vela_lsp_server lifecycle`,
    `cargo test -p vela_lsp_server stdio`,
    `cargo test -p vela_lsp_server tcp`,
    `cargo test -p vela_lsp_server task`,
    ...
    `cargo fmt --all -- --check`, and
    `cargo clippy -p vela_lsp_server --all-targets -- -D warnings`.
  - The raw JSON response payload has been removed from `JsonRpcResult`; the
    temporary wrapper now carries only typed `lsp_server::Response` values,
    typed messages, notification batches, or no output. Legacy malformed JSON
  ...
  - The typed references snapshot request handler now returns
    ...
    `cargo test -p vela_lsp_server handlers::dispatch`.
  - The typed document-highlight snapshot request handler now returns
    `lsp_server::Message` batches directly through the typed worker snapshot
    dispatcher branch. Validated with
    `cargo test -p vela_lsp_server document_highlight`,
    `cargo test -p vela_lsp_server typed_document_highlight`, and
    `cargo test -p vela_lsp_server handlers::dispatch`.
  - The typed document-symbol snapshot request handler now returns
  ...
  - The typed folding-range snapshot request handler now returns
    ...
    `cargo test -p vela_lsp_server handlers::dispatch`, and
    `cargo test -p vela_lsp_server task`.
  - The typed selection-range snapshot request handler now returns
    `lsp_server::Message` batches directly through the typed worker snapshot
    dispatcher branch. Validated with
    `cargo test -p vela_lsp_server selection_range`,
  ...
  - The typed prepare-rename snapshot request handler now returns
    ...
    `cargo test -p vela_lsp_server handlers::dispatch`.
  - The typed rename snapshot request handler now returns
    `lsp_server::Message` batches directly through the typed worker snapshot
    dispatcher branch. Validated with
    `cargo test -p vela_lsp_server rename`,
  ...
  - The typed call-hierarchy prepare, incoming-calls, and outgoing-calls
    ...
    `cargo test -p vela_lsp_server handlers::dispatch`.
  - The typed code-action snapshot request handler now returns
    `lsp_server::Message` batches directly through the typed worker snapshot
    dispatcher branch. Validated with
    `cargo test -p vela_lsp_server code_action`,
    `cargo test -p vela_lsp_server typed_code_action`, and
    `cargo test -p vela_lsp_server handlers::dispatch`.
  - The typed semanticTokens/range snapshot request handler now returns
    `lsp_server::Message` batches directly through the typed worker snapshot
    dispatcher branch. Validated with
...
- [~] Delete obsolete custom JSON-RPC code.
  ...
  - Converted the selection-range feature fixtures off `LspServer::handle_json`
  ...
  - Converted the map-key completion feature fixture off
    ...
    helpers.
  - Converted the struct-body completion feature fixture off
    `LspServer::handle_json` and onto the shared typed request/notification
    helpers.
  - Converted the type-label completion feature fixture off
    `LspServer::handle_json` and onto the shared typed request/notification
    helpers.
  - Converted the dynamic hover degradation fixture off
    `LspServer::handle_json` and onto the shared typed request/notification
    helpers, using a crate-root test import instead of `super::super`.
  - Converted the dynamic signature-help degradation fixture off
    `LspServer::handle_json` and onto the shared typed request/notification
    helpers, using a crate-root test import instead of `super::super`.
  - Converted the dynamic type-definition degradation fixture off
    `LspServer::handle_json` and onto the shared typed request/notification
    helpers, using crate-root test paths instead of `super::super`.
  - Converted the dynamic definition/declaration degradation fixture off
    `LspServer::handle_json` and onto the shared typed request/notification
    helpers.
  - Converted the aliased-import rename fixture off `LspServer::handle_json`
    and onto the shared typed request/notification helpers.
  - Converted the incremental document-change rebuild fixture off
    `LspServer::handle_json` and onto the shared typed request/notification
    helpers.
  - Converted the dynamic/unresolved references degradation fixture off
    `LspServer::handle_json` and onto the shared typed request/notification
    helpers.
  - Converted the cross-file hover fixture off `LspServer::handle_json` and
    onto the shared typed request/notification helpers.
  ...
  - Converted the imported-module references/highlights fixture off
    `LspServer::handle_json` and onto the shared typed request/notification
    helpers.
  - Converted the schema-method-return semantic-token fixture off
    `LspServer::handle_json` and onto the shared typed request/notification
    helpers.
  ...
  - Converted the schema variant-field references fixture off
    `LspServer::handle_json` and onto the shared typed request/notification
    helpers.
  - Converted the source-return-receiver rename fixture off
    `LspServer::handle_json` and onto the shared typed request/notification
    helpers.
  - Converted the imported-function-alias call-hierarchy fixture off
    `LspServer::handle_json` and onto the shared typed request/notification
    helpers.
  - Converted the imported-source inlay fixture off `LspServer::handle_json`
    and onto the shared typed request/notification helpers, using a crate-root
    test import instead of `super::super`.
  ...
  - Converted the schema-field references fixture off
    `LspServer::handle_json` and onto the shared typed request/notification
    helpers.
  - Converted the schema-method-return hover fixture off
    `LspServer::handle_json` and onto the shared typed request/notification
    helpers.
  - Converted the source-method-return call-hierarchy fixture off
    `LspServer::handle_json` and onto the shared typed request/notification
    helpers.
  ...
  - Converted the semantic-token missing-schema degradation fixture off
    `LspServer::handle_json` and onto the shared typed request/notification
    helpers.
  - Converted the source-method-return references fixture off
    `LspServer::handle_json` and onto the shared typed request/notification
    helpers.
  - Converted the imported value/type references fixture off
    `LspServer::handle_json` and onto the shared typed request/notification
    helpers.
  - Converted the source-backed schema rename fixture off
    `LspServer::handle_json` and onto the shared typed request/notification
    helpers.
  - Converted the rename collision fixture off `LspServer::handle_json` and
    onto the shared typed request/notification helpers.
  - Converted the schema method-return reference/highlight fixture off
    `LspServer::handle_json` and onto the shared typed request/notification
    helpers.
  - Converted the imported type-definition fixture off
    `LspServer::handle_json` and the prohibited multi-level `super` import
    onto the shared typed request/notification helpers.
  - Converted the cross-file call hierarchy fixture off
    `LspServer::handle_json` and onto the shared typed request/notification
    helpers.
  - Converted the schema function-return call hierarchy fixture off
    `LspServer::handle_json` and onto the shared typed request/notification
    helpers.
  - Converted the schema reload/configuration fixture off
    `LspServer::handle_json` and onto the shared typed request/notification
    helpers.
  - Converted the schema method-return receiver call hierarchy fixture off
    `LspServer::handle_json` and onto the shared typed request/notification
    helpers.
  ...
  - Converted the completion fixture off `LspServer::handle_json` and onto
    the shared typed request/notification helpers.
  - Converted the member-completion fixture off `LspServer::handle_json` and
    onto the shared typed request/notification helpers.
  ...
  - Converted the cross-file references fixture off `LspServer::handle_json`
    and onto the shared typed request/notification helpers.
  - Converted the returned-receiver references/highlight fixture off
    `LspServer::handle_json` and onto the shared typed request/notification
    helpers.
  - Converted the formatting fixture off `LspServer::handle_json` and onto
    the shared typed request/notification helpers.
