...
- [x] Audit lifecycle capabilities against handlers and matrix rows.
  ...
  - Notification-shaped `initialize` messages are no-response no-ops that do
    not mark the server initialized or poison a later valid request.
  ...
  - Notification-shaped `shutdown` messages are no-response no-ops that do
    not mark the server shut down or block a later valid `shutdown` request.
  - After `exit`, the in-memory dispatcher ignores later input, matching the
    process-termination lifecycle even when tests call the server directly.
  - Request-shaped `exit` messages return invalid-request while still ending
...
- [x] Audit document sync, diagnostics, progress, and cancellation fixtures.
  ...
  - Disk snapshot restoration after `textDocument/didClose` now has
    ...
    document-highlight-query, and type-definition-query coverage in addition
    to diagnostics and definition queries.
...
- [x] Audit references, document highlights, and call hierarchy.
  ...
  - W3/W0 document highlights now have protocol coverage proving `didClose`
    restores disk-snapshot highlight ranges after an open overlay is removed.
...
- [x] `definition_follows_schema_method_on_schema_method_return_receiver`
- [x] `declaration_follows_schema_method_on_schema_method_return_receiver`
...
- [x] `lsp_definition_follows_schema_method_on_schema_method_return_receiver`
- [x] `lsp_declaration_follows_schema_method_on_schema_method_return_receiver`
...
- [x] `inlay_hints_show_imported_const_and_global_typefacts`
...
- [x] `lsp_inlay_hints_show_imported_const_and_global_typefacts`
