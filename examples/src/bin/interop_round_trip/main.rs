...
use vela_examples::interop_round_trip_model::{RoundTripPlayer, build_engine};
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let mut player = RoundTripPlayer { level: 1 };
    ...
    println!("interop_round_trip result={result} level={}", player.level);
    Ok(())
}
