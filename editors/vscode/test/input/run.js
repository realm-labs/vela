...
async function run() {
  ...
  try {
    ...
    for (let index = 2; index < process.argv.length; index += 2) {
      if (process.argv[index] !== "--proof" || !process.argv[index + 1]) throw Error("use --proof <ux03-to-ux08-proof-id>");
      const id = process.argv[index + 1];
      if (!/^ux0[345678]-/.test(id) || !contracts.some((item) => item.id === id) || requestedProofs.includes(id))
